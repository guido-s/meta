...
#' excluding one study at a time (see \code{\link{meta-object}} for more
...
#' 
#' @seealso \code{\link{forest.metainf}}, \code{\link{print.metainf}},
#'   \code{\link{cidprop}}
#' 
#' @references
...
#' @examples
#' data(Fleiss1993bin)
#' ma1 <- metabin(d.asp, n.asp, d.plac, n.plac,
...
#' 
#' forest(metainf(ma1))
#' forest(metainf(ma1, pooled = "random"))
#' 
#' \dontrun{
#' # Create a PDF file with a forest plot containing UTF-8 characters
#' # for tau^2, tau, and I^2
#' forest(metainf(ma1, pooled = "random"),
#'   label.tau2 = "\u03c4\u00b2", label.tau = "\u03c4", label.I2 = "I\u00b2",
#'   file = "forest_with_unicode.pdf", width = 8.5, func.gr = "cairo_pdf")
#' }
#' 
#' metainf(ma1, sortvar = study)
#' metainf(ma1, sortvar = 7:1)
#' 
#' ma2 <- update(ma1, title = "Fleiss1993bin meta-analysis", backtransf = FALSE)
#' metainf(ma2)
#' 
#' data(Fleiss1993cont)
#' ma3 <- metacont(n.psyc, mean.psyc, sd.psyc, n.cont, mean.cont, sd.cont,
...
metainf.meta <- function(x, pooled, sortvar, prediction, overall = x$overall,
                         text.pooled, no = 1,
                         cid = NULL,
  ...
  if (!is.null(x$three.level) && x$three.level)
    stop("Leave-one-out method not implemented for a ",
...
metainf.default <- function(x, ...)
  stop("Leave-one-out meta-analysis not available for an object of class '",
       class(x)[1], "'.",
       call. = FALSE)

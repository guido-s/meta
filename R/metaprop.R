...
#'
#' Cluster-robust variance estimators (Tipton and Pustejovsky, 2015;
#' Pustejovsky and Tipton, 2022) are available for univariate random effects
#' and three-level models (argument \code{method.random.ci} equal to
#' \code{"CR0"}, \code{"CR1"}, or \code{"CR2"}). Internally,
#' \code{\link[metafor]{rma.uni}} or \code{\link[metafor]{rma.mv}} is called
#' and then passed to \code{\link[metafor]{robust}}. For univariate random
#' effects models, study labels (argument \code{studlab}) are used as clusters.
#'
...
#' \bold{73}, 425--35
#'
#' Pustejovsky JE, Tipton E (2022):
#' Meta-Analysis with robust variance estimation: Expanding the range of
#' working models.
...
#' \bold{29}, 3046--67
#'
#' Tipton E, Pustejovsky JE (2015):
#' Small-sample adjustments for tests of moderators and model fit using
#' robust variance estimation in meta-regression.
...
#' # proportions
#' #
#' ma1 <- metaprop(4:1, 10 * 1:4, method = "Inverse")
#' ma2 <- update(ma1, sm = "PAS")
#' ma3 <- update(ma1, sm = "PRAW")
#' ma4 <- update(ma1, sm = "PLN")
#' ma5 <- update(ma1, sm = "PFT")
#' #
#' ma1
#' ma2
#' ma3
...
#' #
#' ma6  <- metaprop(4:1, c(10, 20, 30, 40), method = "Inverse")
#' ma7  <- update(ma6, sm = "PAS")
#' ma8  <- update(ma6, sm = "PRAW")
#' ma9  <- update(ma6, sm = "PLN")
#' ma10 <- update(ma6, sm = "PFT")
#' #
#' forest(ma6)
#' \dontrun{
#' forest(ma7)
#' forest(ma8)
#' forest(ma8, pscale = 100)
#' forest(ma9)
#' forest(ma10)
#' }
#'
...
#' ma12 <- metaprop(c(0, 0, 10, 10), rep(100, 4), incr = 0.1, method = "Inverse")
#' #
#' ma11
#' ma12
#' #
...
#' n <- c(263, 148, 20, 29)
#' #
#' ma14 <- metaprop(event, n, method.ci = "SA", method = "Inverse")
#' ma15 <- update(ma14, method.ci = "SACC")
#' ma16 <- update(ma14, method.ci = "WS")
#' ma17 <- update(ma14, method.ci = "WSCC")
#' ma18 <- update(ma14, method.ci = "CP")
#' #
#' lower <- round(logit2p(rbind(NA, ma14$lower, ma15$lower, NA, ma16$lower,
#'   ma17$lower, NA, ma18$lower)), 4)
#' upper <- round(logit2p(rbind(NA, ma14$upper, ma15$upper, NA, ma16$upper,
#'   ma17$upper, NA, ma18$upper)), 4)
#' #
#' tab1 <- data.frame(
